...
int main(int argc, char** argv) {
    ...
    if(size == 1 || size >= N){
        if(rank == 0){
            ...
            fread(buf, sizeof(float), N, f_in);
            //MPI_File_read_at(f_in, 0, buf, N, MPI_FLOAT, MPI_STATUS_IGNORE);
            printf("read done\n");
            std::sort(buf, buf + N);
            //print_array(buf, N);
            fwrite(buf, sizeof(float), N, f_out);
            //MPI_File_write_at(f_out, 0, buf, N, MPI_FLOAT, MPI_STATUS_IGNORE);
            printf("write done\n");
            MPI_Finalize();
            return 0;
        }else{
    ...
    //if(rank == 1) print_array(my_buf, buf_size);
