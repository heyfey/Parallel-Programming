...
void mandelbrot_set(int rank, int size, int iters, double left, double right, double lower, double upper, int width, int height, int* image_row, int num_threads, int j) {
    ...
    int width1 = (width >> 1) << 1; // make sure the loop is aligned.
#pragma omp parallel for schedule(dynamic, 1) num_threads(num_threads)
    for (int i = 0; i < width1; i += 2) {
        double x01 = i * ((right - left) / width) + left;  
        double x02 = (i+1) * ((right - left) / width) + left;
        ...
        while (1) {
            if (repeats_A >= iters || repeats_B >= iters) break;
        ...
        image_row[i] = repeats_B;
        image_row[i+1] = repeats_A;
    }
